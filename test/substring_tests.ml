...
let test_can_get_a_substring_from_start_when_zipper_is_at_middle _ =
  let ropeSub = Piece_rope.insert (String.length text / 2) insText initialTable
  |> Piece_rope.substring 0 5 in
  let expected = String.sub text 0 5 in
  assert_equal ~printer:print_text expected ropeSub

let test_can_get_substring_of_middle_when_zipper_is_at_middle _ =
  let ropeSub = Piece_rope.insert (String.length text / 2) insText initialTable
  |> Piece_rope.substring (String.length text / 2) (String.length insText) in
  let expected = insText in
  assert_equal ~printer:print_text expected ropeSub

let test_can_get_a_substring_over_a_split_piece _ =
  let ropeSub = Piece_rope.create "123456789"
  |> Piece_rope.insert 0 "a"
  |> Piece_rope.insert 2 "b"
  |> Piece_rope.substring 0 3 in
  let expected = "a1b" in
  assert_equal ~printer:print_text expected ropeSub

let text_edit_1 _ =
  let rope = Piece_rope.create "124567893"
  |> Piece_rope.insert 8 "98"
  |> Piece_rope.insert 9 "ab" in
...
let text_edit_2 _ =
  let rope = Piece_rope.create "123456789"
  |> Piece_rope.insert 8 "a"
  |> Piece_rope.insert 9 "b"
  |> Piece_rope.insert 7 "c"
  |> Piece_rope.insert 8 "d"
  |> Piece_rope.insert 6 "e"
  |> Piece_rope.insert 7 "f" in
...
let test_suite = 
  "Substring_tests" >::: [
    ...
    "test_can_get_a_substring_from_start_when_zipper_is_at_middle " >:: test_can_get_a_substring_from_start_when_zipper_is_at_middle ;
    "test_can_get_substring_of_middle_when_zipper_is_at_middle" >:: test_can_get_substring_of_middle_when_zipper_is_at_middle ;
    "test_can_get_a_substring_over_a_split_piece" >:: test_can_get_a_substring_over_a_split_piece ;
