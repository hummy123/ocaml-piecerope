    ...
    if utf8_pos >= String.length str then
      ...
      let line_breaks =
        if chr = '\r' || (chr = '\n' && not prev_is_cr) then
          ...
          line_breaks
      in
      get (utf8_pos + utf8_length) (utf16_cntr + utf16_length) (utf32_cntr + 1) line_breaks (chr = '\n')
  in
  get 0 0 0 [] false
...
let utf32_sub (str: string) (start: int) (length: int) =
  ...
  let rec sub str_pos cd_pos str_start str_finish =
    if str_pos = String.length str then
      ...
      let str_start = 
        if cd_pos = start then
          str_pos
        else
          str_start
      in
      if cd_pos = finish then
        String.sub str str_start (str_pos - str_start)
      else
...
(* The count_to functions duplicate some code intentionally. 
   ...
   but I was bothered by additional if-statements on each recursion to handle UTF-8/16 cases when the specified index is inside a code point. *)
let count_to_utf32 (str: string) (count_to: int) =
  let rec cnt utf8_pos utf16_pos utf32_pos =
    if utf32_pos = count_to then
      create_offsets utf8_pos utf16_pos utf32_pos
    else
      let chr = String.unsafe_get str utf8_pos in
