...
let is_at_line_break piece_pos_prev piece_pos_next lines =
  ...
  let rec find n =
    if n >= len then false
      ...
      if cur = piece_pos_prev || cur = piece_pos_next then true else find (n + 1)
  in
  find 0
...
let insert_tree insIndex insNode tree buffer =
  let rec ins curIndex node cont =
    ...
    | PT (_, l, _, v, _, r) ->
        ...
        let difference_u32, rStart, rLength_u32 =
          if is_at_line_break (rStart - 1) rStart v.lines then
            let txt = at_start_and_length (rStart - 1) 2 buffer in
            if txt = "\r\n" then
              (difference_u32 + 1, rStart + 1, rLength_u32 - 1)
            else (difference_u32, rStart, rLength_u32)
          else (difference_u32, rStart, rLength_u32)
        in
