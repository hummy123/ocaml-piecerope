...
let initial_model = { col_num = 0; line_num = 0; offset = 0; text = Piece_rope.empty }

type actions = CaretUp | CaretDown | CaretLeft | CaretRight | TypeChar of char | Enter
      ...
      if model.line_num = 0 then
        ...
        let col_num = 
          if model.col_num <= max_col_num && model.col_num >= 0 then
            model.col_num
          else if model.col_num > max_col_num then
            max_col_num
          else
            0
        in
        let offset = line_offset.utf32_offset + col_num in
        { model with line_num = prev_line; offset = offset; col_num = col_num; }
  | CaretDown -> 
      ...
      if next_line <= stats.lines then
        ...
        in
        let offset = line_offset.utf32_offset + col_num in
        { model with line_num = next_line; offset = offset; col_num = col_num; }
      else
        model
  | CaretLeft -> 
      if model.offset = 0 then 
        ...
        let offset = prev_line.utf32_offset + String.length prev_line.line - 1 in
        let col_num = String.length prev_line.line - 1 in
        { model with col_num = col_num; offset = offset; line_num = model.line_num - 1; }
      else
        { model with col_num = model.col_num - 1; offset = model.offset - 1; }
  | CaretRight -> 
      let stats = Piece_rope.stats model.text in
      let line_offset = Piece_rope.get_line model.line_num model.text in
      let line_length = String.length line_offset.line - 1 in
      if model.offset = stats.utf32_length then
        model
      else
        if model.col_num = line_length && model.line_num < stats.lines then
          let next_line_offset = Piece_rope.get_line (model.line_num + 1) model.text in
          let offset = next_line_offset.utf32_offset in
          { model with line_num = model.line_num + 1; offset = offset; col_num = 0; }
        else
          { model with col_num = model.col_num + 1; offset = model.offset + 1; }
  | TypeChar chr ->
      let rope = Piece_rope.insert model.offset (String.make 1 chr) model.text in
      { model with col_num = model.col_num + 1; text = rope; offset = model.offset + 1; }
  | Enter ->
      let rope = Piece_rope.insert model.offset "\n" model.text in
      { col_num = 0; line_num = model.line_num + 1; text = rope; offset = model.offset + 1; }
...
let rec main t model =
  ...
  let text = List.fold_left (fun acc str -> 
